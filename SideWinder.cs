  ...
  public override Grid CreateMaze(Grid grid)
  {
    ...
    for (int y = height - 1; y > 0; y--)
    {
      for (int x = 0; x < width; x++)
      {
        ...
        if (cell.X == width - 1)
        {
          (grid, visited_cells) = ClosingOutRun(visited_cells, grid, choose_direction);
          break;
        }

        (grid, visited_cells) = Step(visited_cells, grid, choose_direction, cell);
